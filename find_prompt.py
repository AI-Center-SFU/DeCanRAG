...
def evaluate_prompts(excel_file: str, save_file: str, rag, delay: int = 10, bert_lang: str = "ru"):
    """
    Загружает Excel-файл с данными, для каждого запроса (из столбца "Перефразирование")
    ...
    if "Перефразирование" not in df.columns or "Ответ" not in df.columns:
        raise ValueError("В Excel файле должны быть столбцы 'Перефразирование' и 'Ответ'.")
    
    results = []
    # Итерация с progress bar
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Оценка промптов"):
        query = row["Перефразирование"]
